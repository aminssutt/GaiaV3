...
export default function NeckPreview({ onNavigate }) {
  const [showConfirm, setShowConfirm] = useState(false);

  const handlePreviewOnAvatar = () => setShowConfirm(true);
  const handleConfirm = () => {
    setShowConfirm(false);
    onNavigate('neckAvatarPreview');
  };
  ...
  return (
    <div className="accessories-page fade-in" style={{ paddingTop: '20px' }}>
      ...
      {showConfirm && (
        <ConfirmationPopup
          variant="accessoryPreview"
          bodyPart="neck"
          customTitle="Preview On Avatar"
          customMessage="Display the neck massager on the avatar?"
          confirmLabel="Show"
          cancelLabel="Cancel"
